...
const Heropromo = () => {
  ...
  const updateIndex = (newIndex:index) => {
    if (newIndex < 0) {
      newIndex = 0;
  ...
  return (
    <div className="w-full h-[764px] mt-4 relative overflow-hidden flex flex-col justify-center bg-red">
      <div
      ...
      <div className="flex justify-between items-center gap-3 absolute bottom-[53px] left-1/2 bg-violet">
        {items.map((item, index) => {
          return (
            <button
              key={index}
